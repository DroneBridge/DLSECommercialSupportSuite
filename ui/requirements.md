...
- The user interface should support the following features:
  ...
  - When selecting an ESP32 in the user interface, the user shall be able to see the detailed information of the ESP32 including the information from the REST:API `/api/system/info` and `/api/settings`. This information shall be displayed inside the collapsable side on the right of the user interface. The user shall be able to resize this right-side panel by dragging it with the mouse. With no checked devices, the Settings panel edits the inspected ESP32; with checked devices, it edits the checked set, including devices hidden by the current filter. Only settings available with compatible types on every target are offered. Equal values are shown, differing values start blank with a Mixed values hint, and only explicitly edited fields are submitted. Changing the checked set clears pending field edits and notifies the user. Applying settings requires confirmation because every target device reboots.
  ...
  - By default the columns for the parameter: Hostname, IP, Activation Status, Firmware version, Chip, Build Version, DLSE Configured MAVLink Sys ID, FC MAVLink Sys ID, DB APMODE SSID, DB APMODE CHANNEL, DEVICE RSSI, AP-MEASURED RSSI, AP CHANNEL, AP BAND, AP WIFI STANDARD, AP LIVE THROUGHPUT, AP RX RATE, AP TX RATE, Operation, and Progress. AP-MEASURED RSSI is populated from the UniFi client's signal value in dBm and remains blank when no matching wireless client is found. AP channel, band, rates, and standard are populated from UniFi; live throughput uses successive byte-counter samples and unsupported fields display `Unavailable`.
  - The scan settings dialog shall allow a user-supplied UniFi Network API token, gateway origin (default `https://192.168.1.1`), site name (default `default`), and TLS certificate verification choice. The UI shall poll active UniFi wireless clients in the background, show UniFi connection status in the footer immediately to the left of the license-server status, and match AP observations to DLSE devices by MAC address with an IP fallback. Live throughput is calculated from successive UniFi byte-counter samples when available; unsupported gateway fields display `Unavailable`.
  - Optional REST-backed table columns shall include DLSE Mode (`esp32_mode`), Baud (`baud`), CTS/RTS/TX/RX GPIO (`gpio_cts`, `gpio_rts`, `gpio_tx`, `gpio_rx`), LED Control (`led_cont_en`), Current Monitor (`adc_a_en`), Voltage Monitor (`adc_v_en`), DLSE Local UDP Port (`udp_local_port`), DLSE Remote UDP Port (`wifi_brcst_port`), Power Management (`show_pm_en`), DLSE MAVLink Heartbeat (`show_pm_en_hb`), and DLSE MAVLink Sys ID Based On IP (`show_en_syid_ip`). GPIO values shall be displayed numerically; `1` and `0` monitor values shall be displayed as `enabled` and `disabled`.
